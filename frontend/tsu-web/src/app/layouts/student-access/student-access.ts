...
export class StudentAccess {
  ...
  protected register() {
    ...
    this.api.registerStudent(this.registerForm).subscribe({
      next: (response) => {
        this.authState.setSession(response);
        this.feedback.set(response.message);
        void this.router.navigateByUrl('/student-profile');
      },
  ...
  private readError(error: HttpErrorResponse) {
    return typeof error.error === 'string' ? error.error : this.i18n.t('studentAccess.errors.requestFailed');
  }
}
